    ...
    def get_all_tweets_with_creator(cls):
        ...
        for row in results: 
            ...
            one_tweets_author_info = {
                # Any fields that are used in BOTH tables will have their name changed,
                #which depends on the order you put them in the JOIN query,
                # use a print statement in your classmethod to show this
                "id": row['users.id'], 
                "name": row['name'],
                "handle": row['handle'],
                "email": row['email'],
            ...
            # Associate the Tweet class instance with the User class instance by filling in the empty
            # creator attribute in the Tweet class
